...
async function getBlogData(slug) {
  ...
  const res = await fetch(`https://strapi.saiteerth.in/api/blogs?populate=*&filters[slug][$eq]=${slug}`, {
    ...
    <div>
      ...
      <article className=" mt-36 mx-auto px-6 lg:px-12 py-16 font-sans antialiased text-slate-900">
        ...
        <div className="relative w-full aspect-[2/1] rounded-[2rem] overflow-hidden mb-12 shadow-xl border border-gray-100">
          <img
            src={"https://strapi.saiteerth.in" + blog.image.url}
              ...
              return (
                <p key={index} className="text-lg leading-relaxed text-slate-700">
                  {block.children.map((child, i) => {
                    // If child text contains raw HTML (links, tables, etc.), render it as HTML
                    if (/<[a-z][\s\S]*>/i.test(child.text ?? '')) {
                      return <span key={i} dangerouslySetInnerHTML={{ __html: child.text }} />;
                    }
                    return (
